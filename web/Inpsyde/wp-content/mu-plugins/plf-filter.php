...
class Plf_filter {    
    ...
    function plugins_loaded() {
        ...
        //jetpack custom post type 初期化前に定義済みか判定しているのでクリアして初期化動作を実行
        //とりあえず woocommerce, jetpack に対応したが他のプラグインでも事前判定が多いようなら public == true, _bultin != true ならクリアするほうが良いかも（要件等）
		if ( post_type_exists( 'jetpack-comic' ) ) {
            unregister_post_type( 'jetpack-comic' );
        }
		if ( post_type_exists( 'nova_menu_item' ) ) {
            unregister_post_type( 'nova_menu_item' );
        }
		if ( post_type_exists( 'jetpack-portfolio' ) ) {
            unregister_post_type( 'jetpack-portfolio' );
        }
		if ( post_type_exists( 'jetpack-testimonial' ) ) {
            unregister_post_type( 'jetpack-testimonial' );
        }
    }
